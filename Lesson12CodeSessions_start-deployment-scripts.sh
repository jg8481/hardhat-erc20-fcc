...
if [ "$1" == "Start-Deployment-On-A-Specific-Network" ]; then
  echo
  echo "------------------------------------[[[[ Start-Deployment-On-A-Specific-Network ]]]]------------------------------------"
  echo
  echo "This command will run the Start-Deployment-On-A-Specific-Network script. This run started on $TIMESTAMP."
  echo
  killall yarn > /dev/null 2>&1
  killall npm > /dev/null 2>&1
  killall node > /dev/null 2>&1
...
usage_explanation() {
  ...
  echo
  echo "You can view just this help menu again (without triggering any automation) by running 'bash ./Lesson12CodeSessions_start-deployment-scripts.sh -h' or 'bash ./Lesson12CodeSessions_start-deployment-scripts.sh --help'."
  echo
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Stop-Local-Blockchain-Nodes-Clean-Environment"
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Install-Tools-On-MacOS-Or-Linux"
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Deployment-On-A-Specific-Network hardhat"
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Deployment-On-Real-Ethereum-Testnet"
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Hardhat-Test"
  echo
  ...
  echo
  echo "bash ./Lesson12CodeSessions_start-deployment-scripts.sh Stop-Local-Blockchain-Nodes-Clean-Environment && bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Deployment-On-A-Specific-Network hardhat"
  echo
  echo "You can also use the provided Lesson12CodeSessions_start-automation-build-pipeline.sh to run all the above commands in a sequence similar to a build pipeline."
  echo
  echo "bash ./Lesson12CodeSessions_start-automation-build-pipeline.sh"
  echo
  echo
}
