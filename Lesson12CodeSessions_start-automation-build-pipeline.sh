...
echo >> ./logs/hardhat-build-pipeline.log
bash ./Lesson12CodeSessions_start-deployment-scripts.sh Stop-Local-Blockchain-Nodes-Clean-Environment >> ./logs/hardhat-build-pipeline.log &&
echo >> ./logs/hardhat-build-pipeline.log
echo "BUILD STAGE 1 :: INSTALL THE REQUIRED TOOLS" >> ./logs/hardhat-build-pipeline.log
echo >> ./logs/hardhat-build-pipeline.log
bash ./Lesson12CodeSessions_start-deployment-scripts.sh Install-Tools-On-MacOS-Or-Linux >> ./logs/hardhat-build-pipeline.log &&
echo >> ./logs/hardhat-build-pipeline.log
echo "BUILD STAGE 2 :: TEST THE DEPLOYED SMART CONTRACT(S) ON A LOCAL BLOCKCHAIN NODE" >> ./logs/hardhat-build-pipeline.log
echo >> ./logs/hardhat-build-pipeline.log
sleep 2
bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Hardhat-Test >> ./logs/hardhat-build-pipeline.log
echo >> ./logs/hardhat-build-pipeline.log
...
echo >> ./logs/hardhat-build-pipeline.log
sleep 2
bash ./Lesson12CodeSessions_start-deployment-scripts.sh Start-Deployment-On-Real-Ethereum-Testnet >> ./logs/hardhat-build-pipeline.log &&
